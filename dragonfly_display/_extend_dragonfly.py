# coding=utf-8
# import the core dragonfly modules
from dragonfly.model import Model

# import the extension functions
from .model import model_to_vis_set, model_envelope_edges_to_vis_set, \
    model_comparison_to_vis_set, model_opening_projection_to_vis_set

# inject the methods onto the classes
Model.to_vis_set = model_to_vis_set
Model.to_vis_set_envelope_edges = model_envelope_edges_to_vis_set
Model.to_vis_set_comparison = model_comparison_to_vis_set
Model.to_vis_set_opening_projection = model_opening_projection_to_vis_set
